...
export function ImageUpload<R extends Response>({ doUpload, extractLink, onUploadComplete }: ImageUploadProps<R>) {
  ...
  const handleImageUpload = useCallback(
    async (image: File) => {
      ...
      try {
        ...
        if (res.ok && res.body) {
          const url = extractLink(res.body as R)
          setLoading(false)
    ...
    [doUpload, extractLink, onUploadComplete]
  )
    ...
    <div className="space-y-3 h-full">
      ...
      {!!uploadedImagePath && (
        <div className="flex items-center justify-between">
          {/* <Link href={uploadedImagePath} className="text-gray-500 text-xs hover:underline ">
